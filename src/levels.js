...
tutorialFly = (world) => {
    ...
    world.waitFor(() => {
        ...
        upInstruction.instruction = up ? nomangle('按住 [上] 飞行') : '';
        tiltInstruction.instruction = tilt ? nomangle('按住 [左/右] 倾斜') : '';
        downInstruction.instruction = down && !player.simplifiedPhysics ? nomangle('按住 [下] 更快下降') : '';
    });
...
mountainThenCeiling = (world) => {
    ...
    const rescueInstruction = new Instruction(nomangle('囚犯可以被营救'));
